package enums;

public class Tshirt {

    private TshirtSize size;
    private String manufacturer;

    public Tshirt(TshirtSize size, String manufacturer) {
        this.size = size;
        this.manufacturer = manufacturer;
    }

    public static void main(String[] args) {
        Tshirt tshirt = new Tshirt(TshirtSize.L, "Polex");
        Tshirt tshirt2 = new Tshirt(TshirtSize.S, "Polex");
        Tshirt tshirt3 = new Tshirt(TshirtSize.M, "FruitOfTheLoom");

        System.out.println(tshirt.size);
        System.out.println(tshirt.getTshirtSizeDetails());
        System.out.println(tshirt2.getTshirtSizeDetails());
        System.out.println(tshirt3.getTshirtSizeDetails());

    }
        public String getTshirtSizeDetails(){
                return size + " ("+ size.getChestwidth()+", "+ size.getShirtLength()+", "+size.getSleeveLength()+")" + " - " + manufacturer;

        }
}
